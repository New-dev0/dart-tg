...
class RPCResult {
  ...
  static fromReader(BinaryReader reader) {
    ...
    if (innerCode == RpcError.CONSTRUCTOR_ID) {
      return new RPCResult(reqMsgId: msgId, body: null, error: RpcError.fromReader(reader));
    }
    if (innerCode == GZIPPacked.CONSTRUCTOR_ID) {
      return new RPCResult(reqMsgId: msgId, body: GZIPPacked.fromReader(reader).data, error: null);
    }
    reader.seek(-4);
